...
**Merging this code does nothing by itself.** Migration 502 is unapplied, and
...
1. **Approve and apply migration 502** (`scripts/migrations/sql/502_fleet_operational_incidents.sql`) against the shared database. Confirm readback: `SELECT filename FROM schema_migrations WHERE filename = '502_fleet_operational_incidents.sql'`.
...
`scripts/migrations/sql/rollback_502_fleet_operational_incidents.sql` drops all
...
- [ ] **Migration 502 applied** to the shared database.
...
Run `PORT=3004 npm run dev` against a database that has migration 502 applied
...
install scheduler entries, send real notifications, or apply migration 502
during this pass.
