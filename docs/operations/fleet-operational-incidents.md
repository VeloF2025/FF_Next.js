...
**Merging this code does nothing by itself.** Migration 499 is unapplied, and
...
1. **Approve and apply migration 499** (`scripts/migrations/sql/499_fleet_operational_incidents.sql`) against the shared database. Confirm readback: `SELECT filename FROM schema_migrations WHERE filename = '499_fleet_operational_incidents.sql'`.
...
`scripts/migrations/sql/rollback_499_fleet_operational_incidents.sql` drops all
...
- [ ] **Migration 499 applied** to the shared database.
...
Run `PORT=3004 npm run dev` against a database that has migration 499 applied
...
install scheduler entries, send real notifications, or apply migration 499
during this pass.
