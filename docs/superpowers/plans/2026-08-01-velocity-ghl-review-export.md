...
- Create `scripts/migrations/sql/478_velocity_review_export.sql` — consent-source extension and durable ledgers.
- Create `scripts/migrations/sql/preflight_478_velocity_review_export.sql` — read-only shared-DB checks.
- Create `scripts/migrations/sql/rollback_478_velocity_review_export.sql` — guarded rollback preserving consent history.
- Create `tests/migrations/478_velocity_review_export.test.ts` — scratch-schema migration integration test.
...
- Create: `scripts/migrations/sql/478_velocity_review_export.sql`
- Create: `scripts/migrations/sql/preflight_478_velocity_review_export.sql`
- Create: `scripts/migrations/sql/rollback_478_velocity_review_export.sql`
- Create: `src/modules/velocity-review/__tests__/schemaSync.test.ts`
- Create: `tests/migrations/478_velocity_review_export.test.ts`
...
Replace the existing `wa_subscriber_consent_source_chk` constraint with the old values plus the two OneMap evidence sources. Guard constraint creation by catalog lookup so the migration is rerunnable. End by inserting `478_velocity_review_export.sql` into `schema_migrations` with `ON CONFLICT DO NOTHING`.
...
TEST_DATABASE_URL="$TEST_DATABASE_URL" npx vitest run tests/migrations/478_velocity_review_export.test.ts
```
...
```bash
git add scripts/migrations/sql/478_velocity_review_export.sql \
  scripts/migrations/sql/preflight_478_velocity_review_export.sql \
  scripts/migrations/sql/rollback_478_velocity_review_export.sql \
  src/modules/velocity-review/__tests__/schemaSync.test.ts \
  tests/migrations/478_velocity_review_export.test.ts
...
Run the documented dev deploy script. Verify the dry-run endpoint against dev with automation disabled. Do not apply migration 478 to the shared DB without the separate migration approval, because dev and production share it.
...
Approval must name migration 478, production deployment, publishing both GHL workflows, the supervised pilot size/date, the go-live watermark, and installation of the 09:00 cron entry.

- [ ] **Step 2: Apply migration 478 through the approved migration process**
