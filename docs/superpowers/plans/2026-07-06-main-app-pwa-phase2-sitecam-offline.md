...
**Interfaces — Produces:**
- `resetPriorQaCycleForResubmission(dropNumber: string, clientSubmissionId?: string): Promise<void>` — bind `const cid = clientSubmissionId ?? null` as `$2`; (a) record `'client_submission_id', $2` inside the `jsonb_build_object` snapshot; (b) add to the `WHERE`: `AND ($2 IS NULL OR COALESCE(submission_history -> -1 ->> 'client_submission_id', '') <> $2)` so a replay of the same submission is a **strict, atomic no-op** (guard already self-clears its other flags; this closes the interleave edge too). When `clientSubmissionId` is undefined (legacy/online callers), `$2` is `NULL` → `$2 IS NULL` short-circuits the clause TRUE → behaves exactly as today. **NB:** the naive `... <> $2` with `''` for legacy is WRONG — a prior snapshot with no `client_submission_id` gives `COALESCE(...,'') <> ''` = FALSE and would wrongly block the legacy reset; the `$2 IS NULL OR …` form is required.
...
- **Test first (unit, mocked `pool.query`):** calling `resetPriorQaCycleForResubmission('DR1', 'uuid-A')` issues an UPDATE whose params include `'uuid-A'` and whose SQL contains the `$2 IS NULL OR COALESCE(submission_history -> -1 ->> 'client_submission_id'` guard + the snapshot `client_submission_id` key; legacy call (no id) passes `null`.
