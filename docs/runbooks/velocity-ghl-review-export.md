...
Require migration 469, the consent table, and no conflicting Velocity-review rows. Redact connection details and any row-level identifiers from evidence. Applying migration 476 to the shared dev/production database requires explicit approval naming migration 476.
...
Read back migration 476 and the disabled singleton control row before continuing. Do not use the migration command as approval to deploy or activate.
