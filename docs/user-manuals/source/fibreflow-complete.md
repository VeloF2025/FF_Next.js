  ...
  headerTemplate: |-
    ...
    <section>
      <div class="header-line">
        ...
        <span>FibreFlow — Complete User Manual v1.7</span>
      </div>
    </section>
  footerTemplate: |-
    <style>
...
<strong>Last Updated:</strong> 13 March 2026<br/>
...
6. [NOC — Network Operations Centre](#6-noc--network-operations-centre)
...
| **Development** | dev.fibreflow.app | Feature development and testing |

> **Note:** The staging environment (vf.fibreflow.app) was retired in March 2026. It now redirects to production.
...
| **NOC** | Wrench | Network operations and ticket management |
...
Click a section header to expand or collapse it. Sections with a direct link (like Activate or NOC) navigate directly to that module.
...
| **Admin** | Organization admin | All modules, full API permissions |
| **Manager** | Department/project lead | Projects, procurement, reports, team management, full API permissions |
...
FibreFlow uses a dual permission system for comprehensive access control:

- **Role Permissions (RBAC)** — Frontend-driven permissions managed through the Access Control page. Each role (super_admin, admin, manager, technician, viewer) has a defined set of module and tab-level permissions that control what users see in the interface.
- **User Permissions (JSONB)** — API-level permissions stored directly on each user record. These control what actions a user can perform through the API, regardless of the frontend interface.

Admins and managers now have full API permissions by default, ensuring they can perform all operations within their accessible modules without encountering API-level permission blocks.
...
| **Open Tickets** | NOC tickets requiring attention |
...
- **Create Ticket** — Open a NOC ticket
...
- **Pagination** — Meeting list supports pagination for efficient browsing of large meeting histories
- **Server-Side Stats** — Meeting statistics (total count, duration, participants) are calculated server-side for accurate, fast-loading dashboard cards
- **Recording Downloads** — Download Fireflies recordings directly to the FibreFlow server for local playback
- **Audio Player** — Built-in audio player for listening to meeting recordings without leaving FibreFlow
- **Web Player Links** — Click through to the Fireflies web player for the full recording experience
...
#### Search
The PP Data tab includes a search bar for quickly finding records by serial number, drop reference, or other identifiers. Search operates across all visible columns and filters results in real time.

#### My Tickets Filter
When viewing PP Data records that have associated tickets, use the **My Tickets** toggle to show only records linked to tickets assigned to you. This helps technicians focus on their own investigation workload.

#### Bulk Ticket Creation
Unresolved PP Data records can be converted into NOC tickets in bulk:

- **Ticket All Unticketed** — Click to automatically create tickets for all unresolved PP Data records that do not yet have an associated ticket. This ensures every unticketed pre-provisioned record gets investigated, not just those with a "not found" status.

#### Create Individual Tickets
Unresolved PP Data records can also be converted individually into NOC tickets:
...
- **NOC** — NOC ticket photos validated through same workflow
...
- **NOC (Section 6)** — NOC tickets assigned to technicians
...
Construction QA photos are ingested from QField via an automated sync pipeline:

1. Technician captures photos on tablet using QField app
2. Photos are stored in QField's GeoPackage (GPKG) format with geographic metadata
3. The automated **GPKG-to-FibreFlow sync pipeline** extracts photos from QField cloud storage, matches them to construction features by pole ID and coordinates, and imports them into FibreFlow
4. Photos are queued for VLM pre-screening
5. Review queue updated automatically

The sync pipeline runs on a scheduled basis, ensuring new field photos appear in the Construction QA queue without manual intervention. Photos taken outside QField can still be uploaded directly through the inspection form.
...
## 6. NOC — Network Operations Centre

The NOC (Network Operations Centre) module — formerly known as "Maintenance" — manages the complete lifecycle of network operations tickets, including maintenance, installations, faults, site surveys, and DevOps tasks.

> **Note:** A comprehensive standalone NOC manual is available with detailed coverage of all features. This chapter provides an overview — see the dedicated **FibreFlow NOC Module User Manual** for in-depth documentation.

### 6.1 Dashboard

**Navigation:** Sidebar → **NOC** (click section header)

![NOC Dashboard](/help-center/screenshots/15-maintenance-dashboard.png)
*Figure 6.1: NOC Dashboard showing ticket statistics, SLA compliance, and workload*

#### Key Metrics

| Metric | Description |
|--------|-------------|
| **Total Tickets** | All NOC tickets (up to 10,000 supported) |
...
The primary ticket management view with drag-and-drop columns and quick-move buttons.

**Navigation:** Sidebar → **NOC** → **Work Orders** tab

![Kanban Board](/help-center/screenshots/16-maintenance-kanban.png)
*Figure 6.2: NOC Kanban Board with tickets organized by status columns*
...
#### Quick-Move Buttons
Each ticket card on the Kanban board includes quick-move buttons that let you advance or return a ticket to adjacent columns without drag-and-drop. This is especially useful on mobile devices or when working with large ticket volumes.

#### Ticket Types

Tickets are categorized by type, allowing teams to track and filter different kinds of work:

| Type | Description |
|------|-------------|
| **Maintenance** | Scheduled or reactive maintenance work |
| **Installation** | New equipment or service installations |
| **Fault** | Reported network faults requiring investigation |
| **Site Survey** | Pre-work site inspections and assessments |
| **DevOps** | Infrastructure, system, or software-related tasks |

#### Filters

The Kanban board provides several filter options for managing large ticket volumes:

- **Type Filter** — Filter tickets by type (Maintenance, Installation, Fault, Site Survey, DevOps)
- **Source Filter** — Filter by ticket source (Manual, QContact, PP Data, OLT, etc.)
- **My Tickets** — Toggle to show only tickets assigned to the current user
- **Created Date** — Filter tickets by creation date range
- **Search** — Free-text search across ticket number, description, and other fields

> **Tip:** The ticket limit has been increased to 10,000, with the created date filter helping you manage large volumes efficiently. Use the **My Tickets** toggle to focus on your personal workload.
...
When a ticket is assigned to a user, an email notification is automatically sent to the assignee. The notification includes:
- Ticket number and title
- Priority level
- Project context
- Direct link to open the ticket in FibreFlow

This ensures assignees are promptly informed of new work items, even when not actively using FibreFlow.

### 6.4 Verification Steps

Before a ticket can be resolved, verification steps must be completed. The verification process includes:

- **Type-Specific Verification Options** — Different ticket types present relevant verification questions and checklists tailored to the work performed
- **Verification Photo Upload** — Upload photos documenting the completed work as evidence of resolution
- **Lightbox Previews** — Click any verification photo to view it in a full-screen lightbox with zoom capability
- **Clipboard Paste** — Paste images directly from the clipboard into the verification photo upload area, eliminating the need to save files first

### 6.5 OLT Integration

The NOC module integrates with the OLT (Optical Line Terminal) data for enhanced troubleshooting:

- **Clickable Ticket Column** — The OLT Investigate tab in Data Sync displays a **Ticket** column that links directly to associated NOC tickets, providing one-click navigation between OLT data and ticket management
- **Auto-Resolve on Ticket Closure** — When a NOC ticket is closed, any associated OLT investigate records are automatically marked as resolved, keeping both systems in sync without manual updates

### 6.6 Key Features Summary
...
### 6.8 Fault Cause Attribution

Every NOC ticket requires a fault cause — this determines contractor liability:
...
The Procurement module manages the complete purchasing workflow through a structured 10-step process — from Bill of Quantities through quote evaluation and purchase order to goods receipt. The workflow includes a formal quote evaluation step before purchase orders are created, ensuring competitive pricing and proper vendor selection.
...
- **BOQ Utilization Table** — On the BOQ detail page, a utilization table displays how much of each BOQ line item has been requisitioned or ordered, showing outstanding quantities at a glance. This helps procurement teams avoid over-ordering and track material consumption against the original bill of quantities.
...
- **Amount** — Total value in Rands with VAT/tax breakdown displayed (net amount, VAT, and gross total shown for transparency)
...
Click any picking/allocation in the list to view its full detail page, including:
- Complete line item breakdown with serial numbers
- Digital signature status and capture
- Transfer history and timestamps
- Print-ready allocation form

**Stock Transfers:**

Transfer stock between field locations with full traceability:
- Select source and destination locations
- Choose items and quantities to transfer
- Transfer detail pages show complete movement history
...
- **Recording Downloads** — Download Fireflies recordings to FibreFlow for local playback
- **Audio Player** — Listen to recordings directly within FibreFlow
- **Web Player Links** — Quick links to the Fireflies web player
...
| **QContact Sync** | NOC ticket synchronization | System Admin |
...
5. **Comprehensive Reporting** — Enhanced reporting capabilities:
   ...
   - **Search on Investigate Tab**:
     - Free-text search across serial numbers, DR numbers, and other fields
     - Quickly locate specific records within large investigation datasets
...
### Appendix A: What's New in Version 1.7

This section summarizes the major changes and new features since version 1.6 (February 2026).

#### Module Changes (Version 1.7)

| Feature | Module | Description |
|---------|--------|-------------|
| **NOC Rename** | NOC | Maintenance module renamed to NOC (Network Operations Centre) with expanded ticket types |
| **Kanban Quick-Move** | NOC | Quick-move buttons on Kanban board for advancing/returning tickets without drag-and-drop |
| **Ticket Types** | NOC | Five ticket types: Maintenance, Installation, Fault, Site Survey, DevOps |
| **Type/Source Filters** | NOC | Dropdown filters for ticket type and source on the Kanban board |
| **Email Notifications** | NOC | Automatic email notifications sent when tickets are assigned to users |
| **Verification Photos** | NOC | Photo upload with lightbox previews and clipboard paste support for verification steps |
| **Type-Specific Verification** | NOC | Verification steps with options tailored to each ticket type |
| **My Tickets Filter** | NOC | Toggle to show only tickets assigned to the current user |
| **10k Ticket Limit** | NOC | Ticket limit increased to 10,000 with created date filter for managing large volumes |
| **OLT Auto-Resolve** | Data Sync | OLT investigate records auto-resolve when linked NOC ticket is closed |
| **OLT Investigate Search** | Data Sync | Free-text search on the OLT Investigate tab |
| **Meeting Recordings** | Communications | Download Fireflies recordings locally with built-in audio player |
| **Meeting Web Player** | Communications | Direct links to Fireflies web player for each recording |
| **Meeting Pagination** | Communications | Paginated meeting list with server-side statistics |
| **BOQ Utilization** | Procurement | Utilization table on BOQ detail page showing requisitioned/ordered quantities |
| **Quote Evaluation** | Procurement | Formal quote evaluation step in 10-step procurement workflow |
| **Tax/VAT on Approvals** | Procurement | Net, VAT, and gross total displayed on approval amounts |
| **Field Stock Detail Pages** | Procurement | Detail pages for pickings and transfers with full traceability |
| **Bulk PP Ticketing** | Activate | Ticket all unticketed PP records in bulk, not just "not found" status |
| **PP Data Search** | Activate | Search bar on PP Data tab for finding records by serial, DR, etc. |
| **Construction QA Sync** | Field Operations | Automated QField GPKG-to-FibreFlow photo sync pipeline |
| **Dual Permission System** | System | Role permissions (RBAC frontend) + user permissions JSONB (API) working together |
| **Manager/Admin API Access** | System | Managers and admins now have full API permissions by default |

#### Environment Changes (Version 1.7)

- **Staging Retired** — The staging environment (vf.fibreflow.app) was retired on 2026-03-11 and now redirects to production. FibreFlow operates with two environments: Development and Production.

---

### Appendix A-1: What's New in Version 1.5
...
| NOC | Full | Full | Full | Assigned | View | — |
...
| **Master Build Agreement** | Renamed from Master Service Agreement |
| **NOC** | Network Operations Centre — ticket management for maintenance, faults, installations, surveys, and DevOps |
...
> **Note:** Staging (vf.fibreflow.app) was retired in March 2026 and redirects to production.
...
| **NOC** | QContact sync failed | Check Data Sync tab for error details. Try manual sync. |
| **NOC** | Cannot move ticket status | Check required fields (fault cause, verification). Verify RBAC permissions. |
...
*Document generated: 13 March 2026*