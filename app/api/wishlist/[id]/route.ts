...
/**
 * GET /api/wishlist/[id] - Get a single wishlist item with details
 */
export async function GET(
  ...
  try {
    ...
    console.error('Wishlist GET [id] error:', error);
    return NextResponse.json(
...
export async function PUT(
  ...
  try {
    ...
    const body: UpdateWishlistItemInput = await req.json();
    const {
      title,
      description,
      priority,
    ...
    console.error('Wishlist PUT [id] error:', error);
    return NextResponse.json(
      { error: 'Internal server error', details: error?.message },
      { status: 500 }
    );
...
export async function DELETE(
  ...
  try {
    ...
    console.error('Wishlist DELETE [id] error:', error);
    return NextResponse.json(
      { error: 'Internal server error', details: error?.message },
      { status: 500 }
    );
