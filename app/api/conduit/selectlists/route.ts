/**
 * GET  /api/conduit/selectlists          — all lists grouped by list_name (for dropdowns)
 * GET  /api/conduit/selectlists?full=true — full rows with IDs (for admin)
 * POST /api/conduit/selectlists          — add a new entry
 */
...
export async function GET(req: NextRequest) {
  try {
    ...
    log.error('[conduit/selectlists GET]', { err: String(err) });
...
export async function POST(req: NextRequest) {
  try {
    ...
    const [row] = await sql`
      INSERT INTO conduit_selectlists (list_name, value, sort_order)
    ...
    log.error('[conduit/selectlists POST]', { err: String(err) });
