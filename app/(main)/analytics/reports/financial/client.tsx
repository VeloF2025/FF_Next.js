  ...
  return (
    <>
      {REPORT_REGISTRY.length === 0 ? (
        <div className="rounded-lg border border-[var(--ff-border-light)] bg-[var(--ff-bg-secondary)] p-12 text-center text-[var(--ff-text-tertiary)]">
        ...
        <div className="space-y-4">
          {REPORT_REGISTRY.map((report) => {
            ...
            return (
              <div
                key={report.id}
                className="rounded-lg border border-[var(--ff-border-light)] bg-[var(--ff-bg-secondary)] overflow-hidden"
              >
                {/* Card Header */}
                <div className="flex items-center justify-between p-4">
                  <div className="flex items-center gap-3">
                    <BarChart2 className="w-5 h-5 text-[var(--ff-primary)] flex-shrink-0" />
                    <div>
                      <h2 className="text-[var(--ff-text-primary)] font-semibold">{report.name}</h2>
                      <p className="text-sm text-[var(--ff-text-secondary)]">{report.description}</p>
                    </div>
                  </div>
                  <button
                    onClick={() => setExpandedId(isExpanded ? null : report.id)}
                    aria-label={`${isExpanded ? 'Close' : 'View'} ${report.name} report`}
                    aria-expanded={isExpanded}
                    className="flex items-center gap-2 px-3 py-1.5 rounded text-sm font-medium bg-[var(--ff-primary)] hover:bg-[var(--ff-primary-600)] text-[var(--ff-primary-foreground)] transition-colors focus:ring-2 focus:ring-[var(--ff-primary-600)] focus:outline-none"
                  >
                    {isExpanded ? (
                      <>
