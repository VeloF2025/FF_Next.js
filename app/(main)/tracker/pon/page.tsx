...
export default function PonTrackerSubPage() {
  ...
  useEffect(() => {
    async function fetchProjects() {
      try {
        ...
        log.error('PonTrackerSubPage: failed to fetch projects', { err }, 'tracker');
      } finally {
        setLoading(false);
      }
    }
    ...
    <div className="flex flex-col gap-6 p-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-semibold text-slate-100">PON Tracker</h1>
            <p className="text-sm text-slate-400">Manual entry tracker — replaces SharePoint Excel sheets</p>
          </div>
        </div>
