                      ...
                      <td className="p-4 text-right">
                        <div className="flex items-center justify-end gap-2">
                          <div className="w-20 h-2 bg-gray-700 rounded-full overflow-hidden">
                            <div
                              className={`h-full ${
                                Number(budget.utilization_percent ?? 0) > 90 ? 'bg-red-500' :
                                Number(budget.utilization_percent ?? 0) > 70 ? 'bg-yellow-500' : 'bg-green-500'
                              }`}
                              style={{ width: `${Math.min(Number(budget.utilization_percent ?? 0), 100)}%` }}
                            />
                          </div>
                          <span className="text-gray-400 text-sm w-12 text-right">
                            {Number(budget.utilization_percent ?? 0).toFixed(0)}%
                          </span>
                        </div>
                      </td>
