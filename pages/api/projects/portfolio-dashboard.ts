...
import { safeQuery, safeArrayQuery } from '@/lib/db';
...
export default async function handler(
  ...
  try {
    ...
    const countsResult = await safeQuery<{
    ...
    const budgetResult = await safeQuery<{
      total_budget: number;
    ...
    const networkResult = await safeQuery<{
      total_drops: number;
    ...
    const complianceResult = await safeQuery<{
      ...
      async (sql) => sql`
        SELECT
          COALESCE(AVG(compliance_score), 0)::numeric as avg_hs_score,
    ...
    const maintenanceResult = await safeQuery<{
      open_tickets: number;
    ...
    // 6. Get expiring documents counts
    const expiringDocsResult = await safeQuery<{
    ...
    const expiringDocs = expiringDocsResult || {
      count_30_days: 0,
      count_60_days: 0,
      count_90_days: 0,
    };
    ...
    const atRiskResult = await safeQuery<{ at_risk_count: number }>(
      async (sql) => sql`
    ...
    const response: PortfolioDashboardResponse = {
      counts: {
        ...
        atRisk: atRiskResult?.at_risk_count || 0,
      },
      budget: {
        totalBudget,
        totalCommitted,
