...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  return zoneDeliveryResponse(res, async () => {
    ...
    const actor = { userId: user.id, email: user.email, permission: COMMAND_PERMISSIONS.scope };
    return service.updateScope(parseScopeBody(req.body), actor);
  });
}
export default withAuth(withPermission(COMMAND_PERMISSIONS.scope, 'edit')(handler));
