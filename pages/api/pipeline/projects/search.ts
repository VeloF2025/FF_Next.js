...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // Build base query with common SELECT
    // Neon requires using ANY() for array checks, not IN with sql()
    const searchPattern = searchQuery ? `%${searchQuery}%` : null;

    // Simplified query - handle exclusions with ANY() or skip the condition
    let results;

    if (allExcludeIds.length > 0) {
      // When we have IDs to exclude, use NOT (id = ANY($array))
      if (searchQuery) {
        results = await sql`
          SELECT
            pp.id,
          ...
          WHERE pp.is_deleted = false
            AND NOT (pp.id = ANY(${allExcludeIds}))
            AND (
              pp.project_name ILIKE ${searchPattern}
              OR pp.area ILIKE ${searchPattern}
              OR pp.municipality ILIKE ${searchPattern}
            )
