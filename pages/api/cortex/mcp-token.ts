 ...
 *
 *   POST   /api/cortex/mcp-token  → { token, expiresAt }   (mint a 30-day token)
 *   DELETE /api/cortex/mcp-token  → { revoked: true }       (revoke ALL my MCP tokens)
 *
 * Mint issues a long-lived per-user Cortex MCP bearer token for the server-verified
 * FF session user (identity is ALWAYS req.user.email — never client-supplied). The
 * token carries the user's email (which drives the bridge's per-user ACL narrowing)
 * and the `token_use:"mcp"` revocation marker; it grants no extra privilege.
 *
