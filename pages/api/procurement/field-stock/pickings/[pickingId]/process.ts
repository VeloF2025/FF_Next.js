...
/**
 * Verify every picking line has sufficient stock at source (runs inside txn
 * with FOR UPDATE). Error messages name the item and warehouse — the PWA shows
 * them to stores staff, and raw UUIDs are unactionable there. Name lookups run
 * only on failing lines, so the happy path costs no extra queries.
 * Exported for tests.
 */
export async function validateStockAvailability(
  ...
  const itemLabel = async (stockItemId: string): Promise<string> => {
    ...
    return rows[0]?.item_code ?? rows[0]?.name ?? stockItemId;
  };
    ...
    finalPicking = await transaction(async (txn) => {
      ...
      if (pickingType === 'issue' && toHolderId !== null) {
        ...
        for (const line of lines) {
          ...
          // field_stock_movements (NOT stock_movements): these columns exist only
          // there; targeting stock_movements 500'd every non-issue picking — same
          // wrong-table bug the issue path already removed (#1849).
          await txn.query(
