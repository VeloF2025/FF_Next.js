...
async function handler(req: AuthenticatedNextApiRequest, res: NextApiResponse) {
  ...
  try {
    // Two-branch approach to avoid conditional SQL fragments with Neon.
    // Primary: match on assigned_to_user_id (UUID index, preferred).
    // Fallback: additionally surface items where assignee_email matches (legacy records
    // that predate migration 256 may not have assigned_to_user_id backfilled).
