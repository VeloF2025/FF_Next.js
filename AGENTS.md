...
- Runner assignment for a given run: `gh run view <id> --json jobs` (the runner inventory API requires repository administration permission).

⚠️ **`gha-runner-fibreflow.service` (systemd USER unit) is a dead orphan — ignore it.** It reports `inactive (dead)` with `Failed to create a session. The runner registration has been deleted from the server, please re-configure`, which reads exactly like a broken runner. It is not one; `velo-fibreflow-u` above is serving jobs normally. **Do NOT "fix" it with `~/bin/install-gha-runner`** — that registers with `--replace` and can delete the *live* runner's registration, taking FF CI down for real. Verified 2026-08-17: run 32045067240 completed `success` on `velo-fibreflow-u` while the orphan unit was dead throughout.
...
|----------|------|---------|
| `src/**/AGENTS.md` | Quick reference / subtree gotchas (target ≤50 lines) | **Path-scoped** to work under that directory |
