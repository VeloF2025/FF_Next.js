...
# Fleet trip build.
#
# Calls pages/api/cron/fleet-build-trips.ts on localhost. The secret is read from the deploy dir's
...
#   */15 * * * * /home/velo/fibreflow-production/scripts/cron-fleet-build-trips.sh >> /home/velo/logs/fleet-build-trips.log 2>&1
#
...
URL="http://localhost:${PORT}/api/cron/fleet-build-trips"

echo "$LOG_PREFIX === Fleet trip build start (port ${PORT}) ==="
...
if ! RESPONSE=$(curl -sS -f -m 300 -X POST \
  ...
  "$URL"); then
  echo "$LOG_PREFIX ERROR: trip build request failed" >&2
  exit 1
fi
echo "$RESPONSE"
report_run_status "trip build" "$RESPONSE" "$LOG_PREFIX" || exit 1

echo ""
echo "$LOG_PREFIX === Fleet trip build done ==="
