...
# 2026-07-25: raised 77→78 catches — origin/master already at 78 before this branch (drifted further since #2239, landed via a later merged PR without bumping the baseline). Verified via `git stash` on this branch: count is 78 on a clean checkout of origin/master HEAD (c6ef5c9e8), before any of feat/hs-contractor-docs-date-tz's changes. This branch's edits add only `::text` date casts + one test file (0 silent catches).
