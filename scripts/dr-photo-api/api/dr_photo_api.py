...
class DRRecordResponse(BaseModel):
    ...
    installer_name: Optional[str] = None  # fieldnme3 from 1Map - Installer Name
    signup_agent: Optional[str] = None    # fieldnme2 from 1Map - Signup Agent
    # Subscriber contact fields from 1Map (added Jan 2026)
    contact_person_name: Optional[str] = None   # Contact person first name
    contact_person_surname: Optional[str] = None  # Contact person surname
    contact_number: Optional[str] = None   # Contact phone number
    email_address: Optional[str] = None    # Contact email
...
async def lifespan(app: FastAPI):
    ...
    await db.connect()
    # Pre-warm the 1Map per-project web sessions (account-gating fallback) so the
    # first non-Etwatwa DR lookup after startup doesn't pay the cold login cost,
    # which exceeds FibreFlow's 5s fetchOneMapRecord abort. Bounded + non-fatal.
    try:
        from agents.integrations.onemap_specialist_agent import (
            OneMapSpecialistAgent,
            WEB_ACCOUNT_IDS,
        )
        _warm_agent = OneMapSpecialistAgent()
        await asyncio.wait_for(
            asyncio.gather(
                *[_warm_agent._web_session(acct) for acct in WEB_ACCOUNT_IDS],
                return_exceptions=True,
            ),
            timeout=25,
        )
        logger.info(f"🔥 Pre-warmed {len(WEB_ACCOUNT_IDS)} 1Map per-project web sessions")
    except Exception as e:
        logger.warning(f"1Map web session pre-warm skipped (non-fatal): {e}")
    yield
...
async def get_dr_record(dr_number: str):
    ...
    try:
        async with OneMapSpecialistAgent() as agent:
            ...
            # Extract ONT barcode, UPS serial, installer name, and signup agent from ALL property records
            # Each DR can have multiple property records (like transactions)
            ont_barcode = None
            ups_serial = None
            installer_name = None  # fieldnme3 in 1Map
            signup_agent = None    # fieldnme2 in 1Map
            # Subscriber contact fields (added Jan 2026)
            ...
            for prop_record in property_records:
                ...
                if not installer_name and raw_data.get('fieldnme3'):
                    installer_name = raw_data.get('fieldnme3')
                if not signup_agent and raw_data.get('fieldnme2'):
                    signup_agent = raw_data.get('fieldnme2')
                # Stop searching if we found all values
                # Contact fields use 1Map column names: nme, srnme, cell01, email, lang
                if not contact_person_name and raw_data.get("nme"):
                    contact_person_name = raw_data.get("nme")
                if not contact_person_surname and raw_data.get("srnme"):
                    contact_person_surname = raw_data.get("srnme")
                # Try multiple cell fields for phone
                if not contact_number:
                    for cell_field in ["cell01", "cell02", "cell03", "whatsapp", "cnt_cell"]:
                        if raw_data.get(cell_field):
                            contact_number = raw_data.get(cell_field)
                            break
                if not email_address and raw_data.get("email"):
                    email_address = raw_data.get("email")
                if not language and raw_data.get("lang"):
                    language = raw_data.get("lang")
                if ont_barcode and ups_serial and installer_name and signup_agent:
                    break
            ...
            if not installer_name:
                installer_name = main_raw_data.get('fieldnme3')
            if not signup_agent:
