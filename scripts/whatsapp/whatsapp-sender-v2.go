...
type PairingState struct {
	Status       string    `json:"status"` // idle, generating, waiting, connected, failed
...
func initializeClient() error {
	...
	// Add event handler for connection state changes
...
func handlePair(w http.ResponseWriter, r *http.Request) {
	...
	// This removes the device from the store
	...
	client.AddEventHandler(func(evt interface{}) {
		...
		case *whatsmeow.ConnectedEvent:
			...
			pairingState.ErrorMessage = ""
			pairingState.mu.Unlock()
			logger.Infof("✅ Pairing successful! Connected to WhatsApp")
		}
	})
