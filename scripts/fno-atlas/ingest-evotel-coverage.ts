...
const SOURCE_URL = 'https://area-service.national-za.aex.systems/kml?optimiseKml=True&operatorId=aeef38c0-f1e9-4e83-ad4f-3e35aab8dfcf';
const COVERAGE_PAGE_URL = 'https://my.evotel.co.za/Coverage/search';
const USER_AGENT = 'FibreFlow FNO Atlas Evotel KML ingestion/1.0';
...
async function fetchKml(): Promise<string> {
  ...
  if (!response.ok) throw new Error(`Evotel KML fetch failed ${response.status}`);
...
function parseKmlFeatures(kml: string): KmlFeature[] {
  ...
  return placemarks.flatMap((placemark, index) => {
    ...
    const name = textBetween(placemark, 'name') || `Evotel coverage ${index + 1}`;
    ...
    return coordinates.flatMap((match, ringIndex) => {
      ...
      if (!ring) return [];
      return [{
        externalId: `evotel-kml:${index}:${ringIndex}`,
        name,
...
async function upsertSource(client: Client): Promise<string> {
  const source = await client.query<{ id: string }>(
    `INSERT INTO fno_atlas_sources (
     ...
     SELECT id,
       'Evotel official coverage KML',
       $1::text,
       'official_api',
       'api',
       15,
       'Evotel coverage page loads this Automation Exchange KML feed with operatorId aeef38c0-f1e9-4e83-ad4f-3e35aab8dfcf.',
       NOW()
  ...
  if (!source.rows[0]) throw new Error('Operator not seeded: evotel');
  ...
  try {
    ...
    const run = await client.query<{ id: string }>(
      `INSERT INTO fno_atlas_ingestion_runs (
       ...
       ) VALUES ($1, 'started', $2, $3::jsonb, 'fno-atlas-ingest-evotel-coverage')
       RETURNING id`,
      [sourceId, features.length, JSON.stringify({ source: 'evotel-official-kml', featureCount: features.length })],
    );
    const runId = run.rows[0]?.id;
    if (!runId) throw new Error('Could not create Evotel ingestion run');
    ...
    return { source: 'evotel-official-kml', polygons_seen: features.length, polygons_imported: imported, ...counts };
  } catch (error) {
...
async function main(): Promise<void> {
  ...
  if (!write) {
    ...
    writeLine({ source: 'evotel-official-kml', polygons_seen: features.length, ...counts });
    return;
  }
