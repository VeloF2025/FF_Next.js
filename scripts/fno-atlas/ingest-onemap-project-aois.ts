...
const SOURCE_NAME = 'Velocity 1Map area AOIs';
const SOURCE_URL = 'fibreflow://onemap_properties';
const DRY_RUN = process.argv.includes('--dry-run');
const MIN_POINTS = 30;
const CLUSTER_EPS_METRES = 900;
  ...
  if (existing.rows[0]) {
    await client.query(
      ...
      [
        existing.rows[0].id,
        SOURCE_NAME,
        'Internal Velocity/1Map GPS-derived AOI polygons. Area-only layer; does not expose demand points and is not official FNO-published coverage.',
      ],
    );
  ...
  const result = await client.query<{ id: string }>(
    ...
    [
      ...
      JSON.stringify({
        ...
        clusterEpsMetres: CLUSTER_EPS_METRES,
...
async function preview(client: Client): Promise<AoiSummaryRow[]> {
  const result = await client.query<AoiSummaryRow>(
    `WITH valid_points AS (
       ...
       SELECT site_code, geom_3857,
              ST_ClusterDBSCAN(geom_3857, eps := $1, minpoints := 25) OVER (PARTITION BY site_code) AS cluster_id
         FROM valid_points
     ), grouped AS (
       SELECT site_code,
              cluster_id,
              COUNT(*)::int AS point_count,
              ST_Multi(ST_Transform(ST_Buffer(ST_ConcaveHull(ST_Collect(geom_3857), $2, true), $3), 4326)) AS geom
         FROM clustered
        WHERE cluster_id IS NOT NULL
        GROUP BY site_code, cluster_id
       HAVING COUNT(*) >= $4
     )
     SELECT site_code,
            CASE WHEN COUNT(*) OVER (PARTITION BY site_code) = 1
              THEN site_code
              ELSE site_code || ' AOI ' || (ROW_NUMBER() OVER (PARTITION BY site_code ORDER BY point_count DESC))::text
            END AS area_name,
            point_count,
      ...
      LIMIT 200`,
    [CLUSTER_EPS_METRES, HULL_RATIO, BUFFER_METRES, MIN_POINTS],
  );
  return result.rows;
}
...
async function importAois(client: Client, operatorId: string, sourceId: string, runId: string): Promise<number> {
  const result = await client.query<{ inserted: number }>(
    `WITH valid_points AS (
       ...
       SELECT site_code, status, flow_name_groups, geom_3857,
              ST_ClusterDBSCAN(geom_3857, eps := $1, minpoints := 25) OVER (PARTITION BY site_code) AS cluster_id
         FROM valid_points
     ), grouped AS (
       SELECT site_code,
              ...
              ST_Multi(ST_Transform(ST_Buffer(ST_ConcaveHull(ST_Collect(geom_3857), $2, true), $3), 4326)) AS geom
         FROM (
           SELECT site_code, cluster_id, status, geom_3857,
                  COUNT(*) OVER (PARTITION BY site_code, cluster_id, status)::int AS status_count
             FROM clustered
            WHERE cluster_id IS NOT NULL
         ) ranked
        GROUP BY site_code, cluster_id
       ...
       SELECT site_code,
              ...
              jsonb_build_object(
                ...
                'label', 'Velocity 1Map AOI - not official FNO-published coverage'
              ) AS raw_properties
         FROM grouped
        WHERE ST_IsValid(geom)
          AND NOT ST_IsEmpty(geom)
       ...
       SELECT $6, $5, $7, site_code, area_name, 'onemap_site_aoi', point_count,
              confidence, geom, raw_properties, NOW(), NOW()
         FROM named
       RETURNING id
     )
     SELECT COUNT(*)::int AS inserted FROM inserted`,
    [CLUSTER_EPS_METRES, HULL_RATIO, BUFFER_METRES, MIN_POINTS, sourceId, operatorId, runId],
  );
...
async function main() {
  ...
  try {
    ...
    console.log(`1Map AOI preview: ${rows.length} area polygons would be generated`);
    ...
    console.log(`Inserted ${inserted} Velocity 1Map AOI area polygons`);
  } catch (error) {
