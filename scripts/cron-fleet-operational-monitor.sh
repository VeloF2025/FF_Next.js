#!/usr/bin/env bash
# Five-minute Fleet operational-incident monitor.
#
...
#   */5 * * * * /home/velo/fibreflow-production/scripts/cron-fleet-operational-monitor.sh >> /home/velo/logs/fleet-operational-monitor.log 2>&1
#
...
URL="http://localhost:${PORT}/api/cron/fleet-operational-monitor"

echo "$LOG_PREFIX === Fleet operational monitor start (port ${PORT}) ==="
...
echo "$RESPONSE"
report_run_status "operational monitor" "$RESPONSE" "$LOG_PREFIX" || exit 1

echo ""
echo "$LOG_PREFIX === Fleet operational monitor done ==="
