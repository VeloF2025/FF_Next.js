-- Rollback PR-8 — restores migration 364's broken trigger body (done_at).
-- Run only to revert the fix; trigger will silently no-op again until forward
-- migration is reapplied or another fix lands.
...
DELETE FROM migrations WHERE version = '366';
