...
async function backfillPickingEvents(pool: Pool, commit: boolean): Promise<number> {
  ...
  const r = await pool.query(`
    ...
    SELECT u.serial_id,
           ...
           sp.id,
           sp.technician_id,
           -- Prod stock_pickings has no done_at column. Use the technician's
           -- signed_at (commitment timestamp) → effective_date → approved_at
           -- as the issue moment; falls back to NOW() for fully blank rows.
           COALESCE(sp.signed_at, sp.effective_date, sp.approved_at, NOW()),
