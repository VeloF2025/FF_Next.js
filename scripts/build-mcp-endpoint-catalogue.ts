...
import { MCP_DENIED_GROUPS, MCP_DENIED_PATHS } from '../src/lib/auth/mcpDeniedAreas';
 ...
 */
// The list itself now lives in src/lib/auth/mcpDeniedAreas.ts, where it is ENFORCED for
// every kind='mcp' session in withAuth. Withholding these from the catalogue is still
// worth doing — it stops the model proposing a path it would only be refused, saving a
// round-trip — but it is no longer the control. Importing rather than restating is the
// point: the previous copy silently disagreed with the runtime guard for a week.
export const DENIED_GROUPS = new Set(MCP_DENIED_GROUPS);
 ...
 */
export const DENIED_PATHS = [...MCP_DENIED_PATHS];
