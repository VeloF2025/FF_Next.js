> **Archived record.** The live-state claims below — production status, deploy SHAs, the
> `python3 -m ff_mcp` PID, worktree and stash counts — are as of 2026-07-29 12:45 and are
> known to have changed since. Read `.claude/handoffs/LATEST.md` for current values before
> acting on any of them. The defect write-ups and lessons below are not time-sensitive.
