...
export async function computeProjectSummary(projectId: string): Promise<ProjectSummary | null> {
  try {
    ...
    return {
      ...
      permissions_complete: row.poles_complete ?? 0,
      pct_permissions: (row.poles_complete ?? 0) / polesScope,
      poles_scope: polesScope,
      poles_complete: row.poles_complete ?? 0,
      pct_poles: (row.poles_complete ?? 0) / polesScope,
      signups_scope: signupsScope,
      signups_complete: row.signups_complete ?? 0,
      pct_signups: (row.signups_complete ?? 0) / signupsScope,
      cwc_scope: cwcScope,
      cwc_complete: row.cwc_complete ?? 0,
      pct_cwc: (row.cwc_complete ?? 0) / cwcScope,
      optical_scope: opticalScope,
      optical_complete: row.optical_complete ?? 0,
      pct_optical: (row.optical_complete ?? 0) / opticalScope,
      connected_scope: connectedScope,
      connected_complete: row.connected_complete ?? 0,
      pct_connected: (row.connected_complete ?? 0) / connectedScope,
    };
  } catch (err) {
