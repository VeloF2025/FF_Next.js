 ...
 * IndexedDB stores are namespaced `SiteCamJobDB:<jobType>:<siteId>` (Task 2),
 * so this tries each `jobType` in turn and returns whichever (if either)
 * already has durable job meta for this site — in practice at most one ever
 * will, since a given site is either an activation DR or a civil pole.
 */
...
export async function findRestorableSiteCamJob(siteId: string): Promise<RestorableSiteCamJob | null> {
  for (const jobType of JOB_TYPES) {
    try {
      const meta = await new SiteCamPhotoStore(jobType, siteId).getMeta();
      if (meta) return { jobType, siteInfo: meta.siteInfo };
    } catch (err) {
      log.warn('SiteCam job restore lookup failed', { jobType, siteId, err: String(err) }, MODULE);
    }
  }
  return null;
}
