 ...
 *
 * Creates GitHub Issues in VelocityFibre/mvp-builds when wishlist items
...
interface WishlistItemForGitHub {
  id: string;
  title: string;
  description?: string;
  priority: string;
 ...
 */
export function generateSpecTemplate(item: WishlistItemForGitHub): string {
  const lines: string[] = [];
  ...
  lines.push(`|-------|-------|`);
  lines.push(`| Wishlist ID | \`${item.id}\` |`);
  ...
  lines.push('---');
  lines.push('_Auto-generated from FibreFlow Wishlist_');
  lines.push('');
...
export async function createMvpIssue(
  ...
  try {
    ...
    log.info(`Created GitHub Issue #${issue.number} for wishlist item ${item.id}`, 'GitHubMvpSync');
