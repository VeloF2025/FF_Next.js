/**
 * Custom hook for devQueue data management
 */
...
export function useDevQueue() {
  ...
  const fetchBoard = useCallback(async () => {
    ...
    try {
      ...
      const data = await devQueueService.getBoard();
      ...
      const message = err instanceof Error ? err.message : 'Failed to load devQueue';
      setError(message);
  ...
  // Create new item
  const createItem = useCallback(async (input: CreateDevQueueItemInput) => {
    try {
      const newItem = await devQueueService.create(input);
      notificationService.success('DevQueue item created successfully');
  ...
  const moveItem = useCallback(async (itemId: string, targetColumn: string, position: number) => {
    ...
    try {
      ...
      await devQueueService.move({ itemId, targetColumn, position });
    } catch (err) {
      // Revert on error
      await fetchBoard();
  ...
  const voteItem = useCallback(async (itemId: string) => {
    try {
      const result = await devQueueService.vote(itemId);
  ...
  const deleteItem = useCallback(async (itemId: string) => {
    try {
      await devQueueService.delete(itemId);
  ...
  // Update item
  const updateItem = useCallback(async (itemId: string, updates: Parameters<typeof devQueueService.update>[1]) => {
    try {
      const updatedItem = await devQueueService.update(itemId, updates);
