...
import type { DevQueueWorkType } from '../types/devQueue';
...
/**
 * Trigger build for a devQueue item (POC or Harness)
 */
  ...
  try {
    ...
    const response = await fetch(`${HARNESS_TRIGGER_URL}/api/mvp-pipeline/trigger`, {
      ...
      headers: {
        ...
        'Authorization': `Bearer ${HARNESS_TRIGGER_SECRET}`,
      },
 ...
 */
export function getWorkTypeLabel(workType: DevQueueWorkType): string {
  const labels: Record<DevQueueWorkType, string> = {
    feature: '✨ New Feature',
    fix: '🐛 Bug Fix',
    amendment: '📝 Amendment',
    refactor: '🔧 Refactor',
