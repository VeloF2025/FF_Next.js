...
export async function sendWhatsAppText(opts: {
  ...
  // WAHA 1:1 DM via the dependency-free ./wahaDmClient. Signature is
  // sendWahaDm(phone, message): Promise<void> — it throws on HTTP failure and
  // returns no provider message id. So a clean resolve means success (no
  // providerMessageId); a throw means the send failed.
  try {
    await sendWahaDm(opts.toPhone, opts.message);
