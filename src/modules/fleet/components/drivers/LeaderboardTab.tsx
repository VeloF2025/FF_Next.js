/**
 * Fleet Drivers Leaderboard Tab
 * Performance rankings extracted from existing leaderboard page
 */
...
export function LeaderboardTab({ onViewScorecard }: LeaderboardTabProps) {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-[var(--ff-text-secondary)]">
            Performance based on check-ins, fuel efficiency, authorization, and vehicle care
          </p>
        </div>
        ...
        <div className="space-y-4">
          {leaderboard.entries.map((entry) => (
            <LeaderboardRow
              key={entry.staffId}
              entry={entry}
              onViewScorecard={() => onViewScorecard(entry.staffId)}
            />
          ))}
        </div>
      )}
