 ...
 * `sql` tag is not used here at all, only `query`/`queryOne`).
 *
 * The column lists, row shapes and row→domain mapping live in
 * `reviewQueryRows.ts`; this file owns the predicates, the batching and the
 * assembly.
 */
...
import {
  DETAIL_COLUMNS, EVIDENCE_COUNT_SUBQUERY, LIST_COLUMNS, iso, isoOrNull, mapDetailRow, mapListRow,
  type DetailRow, type IncidentDetailCore, type ListRow,
} from './reviewQueryRows';
import type {
  IncidentAction, IncidentActionType, IncidentCorrectionLink, IncidentDeliverySummary, IncidentDriverInputSummary,
  IncidentEvidence, IncidentEvidenceType, IncidentListItem, IncidentListRequest, IncidentListResult, IncidentLifecycleStatus,
  IncidentVisibility, SanitizedIncidentMetadata,
} from './types';

export type { IncidentDetailCore } from './reviewQueryRows';
