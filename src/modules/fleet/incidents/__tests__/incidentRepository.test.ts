...
describe('append-only action and evidence writes', () => {
  ...
  it('never writes visibility for a manager-authored action — it relies on migration 506\'s column DEFAULT \'internal\' and reads back whatever the row carries', async () => {
    const txn = fakeTxn();
    txn.queryOne.mockResolvedValue({
  ...
  it('reads back the evidence row\'s visibility (migration 506 default \'internal\')', async () => {
    const txn = fakeTxn();
    txn.queryOne.mockResolvedValue({
