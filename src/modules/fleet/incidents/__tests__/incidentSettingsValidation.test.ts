...
describe('parseUserSearchQuery', () => {
  ...
  it('parses a trimmed search term', () => {
    expect(parseUserSearchQuery({ search: '  Nomvula  ' })).toEqual({ mode: 'search', search: 'Nomvula' });
  });

  it('rejects a missing search when no ids are given', () => {
    expect(() => parseUserSearchQuery({})).toThrow(IncidentValidationError);
    expect(() => parseUserSearchQuery({ search: '   ' })).toThrow(IncidentValidationError);
  });

  it('parses a comma-separated ids list, trimming whitespace', () => {
    expect(parseUserSearchQuery({ ids: ` ${idA} , ${idB} ` })).toEqual({ mode: 'ids', ids: [idA, idB] });
  });

  it('prefers ids over search when both are present', () => {
    expect(parseUserSearchQuery({ ids: idA, search: 'Nomvula' })).toEqual({ mode: 'ids', ids: [idA] });
  });

  it('rejects an ids list containing an invalid UUID', () => {
    expect(() => parseUserSearchQuery({ ids: `${idA},not-a-uuid` })).toThrow(IncidentValidationError);
  });

  it('rejects an ids list that is empty after trimming', () => {
    expect(() => parseUserSearchQuery({ ids: ' , ' })).toThrow(IncidentValidationError);
  });

  it('rejects a non-string search or ids value (array query params)', () => {
    expect(() => parseUserSearchQuery({ search: ['a', 'b'] })).toThrow(IncidentValidationError);
    expect(() => parseUserSearchQuery({ ids: ['a', 'b'] })).toThrow(IncidentValidationError);
  });
});
