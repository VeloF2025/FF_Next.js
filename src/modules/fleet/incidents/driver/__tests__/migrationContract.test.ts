...
const migrationPath = resolve(process.cwd(), 'scripts/migrations/sql/506_fleet_incident_driver_input.sql');
const rollbackPath = resolve(process.cwd(), 'scripts/migrations/sql/rollback_506_fleet_incident_driver_input.sql');
...
describe('fleet incident driver-input migration contract', () => {
  ...
  it('rolls back only PR7 objects and reverts the PR6 visibility extension in dependency-safe order', () => {
    ...
    expect(sql).toContain("filename = '506_fleet_incident_driver_input.sql'");
