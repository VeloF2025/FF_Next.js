...
// Mirrors migration 507's `fleet_incident_driver_input_settings` seed
// (`evidence_allowed_mime_types` default) — see `scripts/migrations/sql/507_fleet_incident_driver_input.sql`.
...
describe('configured MIME/byte limits', () => {
  ...
  it('every mime type the MIGRATION seeds has a registered content-signature — otherwise every upload of it fails closed at runtime', () => {
    ...
    const migration = readFileSync(
      resolve(process.cwd(), 'scripts/migrations/sql/507_fleet_incident_driver_input.sql'), 'utf8');
