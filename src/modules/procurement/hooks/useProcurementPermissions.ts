  ...
  return useMemo(() => {
    ...
    if (!projectId) {
      return {
        ...
        canManagePickings: false,
        canCancelPickings: false,
        canManageReturns: false,
        canReverseMovements: false,
        canManageAccountability: false,
        canViewFaultReports: canReadStock || isAdmin || isManager,
        canManageFaultReports: false,
        canViewAuditLog: isAdmin || isManager,
        canExportAuditLog: false,
        canConfirmGRN: false,
        canRegisterGRNAssets: false,
    ...
    return {
      ...
      canManagePickings: canWriteStock || isAdmin || isManager || isSupervisor,
      canCancelPickings: isAdmin || isManager,
      canManageReturns: canWriteStock || isAdmin || isManager || isSupervisor,
      canReverseMovements: isAdmin || isManager,
      canManageAccountability: isAdmin || isManager,
      canViewFaultReports: canReadStock || isAdmin || isManager || isSupervisor || isTechnician,
      canManageFaultReports: canWriteStock || isAdmin || isManager || isSupervisor,
      canViewAuditLog: isAdmin || isManager || isSupervisor,
      canExportAuditLog: isAdmin || isManager,
      canConfirmGRN: isAdmin || isManager || isSupervisor,
      canRegisterGRNAssets: isAdmin || isManager,
