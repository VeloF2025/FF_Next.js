 ...
 * of serial-tracked stock items. Only serial-tracked items are shown
 * because the downstream ScanSerialsStep requires each unit to have a
 * serial number.
 *
 * Endpoint: GET /api/procurement/field-stock/items?trackingType=serial[&search=…]
 * via fetchSerialStockItems() in api.ts.
 */

import { useState, useEffect, useCallback } from 'react';
import { Search, Package, ChevronRight } from 'lucide-react';
import { fetchSerialStockItems } from '@/modules/field-stock-pwa/api';
...
export function PickItemStep({ technician, onPick }: PickItemStepProps) {
  ...
  const load = useCallback(
    async (searchTerm: string) => {
      ...
      setError(null);
      try {
        const results = await fetchSerialStockItems(
    ...
    <div className="space-y-3">
      ...
      {loading ? (
        ...
        <div className="px-4 py-8 flex flex-col items-center gap-2 text-neutral-500 text-sm rounded-lg bg-neutral-950 border border-neutral-800">
          ...
          <span>{search ? 'No items match your search.' : 'No serial-tracked items found.'}</span>
        </div>
      ) : (
        <ul className="divide-y divide-neutral-800 rounded-lg bg-neutral-950 border border-neutral-800">
          {items.map((item) => (
