...
export async function getVerificationSteps(ticketId: string): Promise<VerificationStep[]> {
  ...
  // LEFT JOIN attachments so the caller gets the full photo gallery per step.
  // json_agg ... FILTER returns NULL when a step has no evidence photos — we
  // coalesce to an empty array so the frontend can treat photos as always
  // present. photo_url stays as the "cover" for back-compat with older code.
