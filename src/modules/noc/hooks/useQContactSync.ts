...
async function fetchSyncStatus(): Promise<SyncStatusOverview> {
  const response = await fetch('/api/noc/sync/qcontact/status', {
    method: 'GET',
    headers: {
...
async function fetchSyncLog(filters?: SyncLogFilters): Promise<SyncLogListResponse> {
  ...
  const url = `/api/noc/sync/qcontact/log${queryParams.toString() ? `?${queryParams.toString()}` : ''}`;
...
async function triggerManualSync(request?: FullSyncRequest): Promise<FullSyncResult> {
  const response = await fetch('/api/noc/sync/qcontact', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
