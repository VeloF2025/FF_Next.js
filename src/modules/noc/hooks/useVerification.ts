 ...
 * Uses flat route /api/noc/verification?ticketId= to avoid Vercel nested route issues
 */
async function fetchVerificationSteps(ticketId: string): Promise<VerificationStep[]> {
  const response = await fetch(`/api/noc/verification?ticketId=${ticketId}`, {
    method: 'GET',
    headers: {
 ...
 * Uses flat route /api/noc/verification-step to avoid Vercel nested route issues
 */
async function updateVerificationStepAPI(
  ...
  const response = await fetch(`/api/noc/verification-step?ticketId=${ticketId}&stepNumber=${stepNumber}`, {
    method: 'PUT',
    headers: {
      'Content-Type': 'application/json',
    },
