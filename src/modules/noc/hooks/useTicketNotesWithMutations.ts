...
async function fetchNotes(
  ...
  const url = `/api/noc/tickets/${ticketId}/notes${params.toString() ? '?' + params.toString() : ''}`;
...
async function createNote(
  ...
  const response = await fetch(`/api/noc/tickets/${ticketId}/notes`, {
    method: 'POST',
...
async function updateNote(
  ...
  const response = await fetch(`/api/noc/tickets/${ticketId}/notes/${noteId}`, {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(payload),
  });
