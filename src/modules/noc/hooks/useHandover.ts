...
async function validateHandoverGateAPI(
  ...
  const response = await fetch(
    `/api/noc/tickets/${ticketId}/handover/validate?type=${handoverType}`,
    {
      method: 'GET',
      headers: {
...
async function createHandoverSnapshotAPI(
  ...
  const response = await fetch(`/api/noc/tickets/${payload.ticket_id}/handover`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
...
async function getHandoverHistoryAPI(ticketId: string): Promise<TicketHandoverHistory> {
  const response = await fetch(`/api/noc/tickets/${ticketId}/handover-history`, {
    method: 'GET',
    headers: {
      'Content-Type': 'application/json',
    },
...
async function getHandoverByIdAPI(handoverId: string): Promise<HandoverSnapshot> {
  const response = await fetch(`/api/noc/handovers/${handoverId}`, {
    method: 'GET',
    headers: {
      'Content-Type': 'application/json',
    },
 ...
 *       router.push(`/noc/handovers/${snapshot.id}`);
 *     },
 *     onError: (error) => {
