 ...
 *
 * Used in:
 * - /maintenance/wa-tracking (standalone page)
 * - /maintenance/data-sync (as a tab)
 */
...
export function WATrackingDashboard() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/maintenance/wa-messages?${params}`);
  ...
  const toggleExpand = async (dropNumber: string) => {
    ...
    try {
      const response = await fetch(`/api/maintenance/wa-dr-photos?dropNumber=${dropNumber}`);
      if (response.ok) {
        const result = await response.json();
