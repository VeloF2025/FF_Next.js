        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {photos.map((photo, idx) => (
            ...
            >
              ...
              <button
                type="button"
                aria-label={`Remove photo ${idx + 1}${photos[idx]?.caption ? `: ${photos[idx].caption}` : ''}`}
                onClick={() => removePhoto(idx)}
                className="absolute top-1 right-1 p-2 bg-red-500/80 rounded-full text-white opacity-0 group-hover:opacity-100 transition-opacity"
              >
