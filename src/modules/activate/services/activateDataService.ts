...
export interface DrListItem {
  ...
  /** Timestamp when auto-feedback was sent to technician (null if not yet sent) */
  autoFeedbackSentAt: string | null;
}
...
export async function fetchDrops(filters: DropsFilters = {}): Promise<DropsApiResponse> {
  ...
  const transformedDrops: DrListItem[] = data.data.map((drop: any) => {
    ...
    return {
      ...
      oesImportedAt: drop.oes_imported_at || null,
      ...
      autoFeedbackSentAt: drop.auto_feedback_sent_at || null,
    };
  });
