/**
 * PPDataCardModal — full-screen drill-down for a single PP status category.
 * Shows a filterable + selectable table with bulk ticket creation.
 */
...
const CATEGORY_CONFIG: Record<PPCardCategory, { title: string; statusFilter: string; color: string }> = {
  ...
  activated: { title: 'Activated',       statusFilter: 'activated', color: 'text-green-400' },
  located:   { title: 'Located',         statusFilter: 'located',   color: 'text-blue-400' },
  not_found: { title: 'Not Found',       statusFilter: 'not_found', color: 'text-amber-400' },
  ticketed:  { title: 'Ticketed',        statusFilter: 'ticketed',  color: 'text-orange-400' },
};

interface PPDataCardModalProps { category: PPCardCategory; count: number; onClose: () => void; }
...
export function PPDataCardModal({ category, count, onClose }: PPDataCardModalProps) {
  ...
  const fetchRecords = useCallback(async () => {
    ...
    try {
      ...
      if (data.success) { setRecords(data.data); setTotalPages(data.pagination.totalPages); setTotal(data.pagination.total); }
    } catch (err) { log.error('Failed to fetch records', { err }, 'PPDataCardModal'); }
  ...
  const toggleSelectAll = () => {
    if (allSelectableChecked) { setSelectedIds(prev => prev.filter(id => !selectableOnPage.some(r => r.id === id))); }
    else { setSelectedIds(prev => { const s = new Set(prev); selectableOnPage.forEach(r => s.add(r.id)); return Array.from(s); }); }
  };

  const handleCreateTickets = async (params: { ticket_type: string; ticket_category: string; priority: string; notes: string; batches: PPTicketBatch[] }) => {
    setCreatingTickets(true);
    try {
      const res = await fetch('/api/activate/pp-data-tickets', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ batches: params.batches.map(b => ({ pp_data_ids: b.pp_data_ids, assigned_team_id: b.assigned_team_id })), ticket_type: params.ticket_type, ticket_category: params.ticket_category, priority: params.priority, notes: params.notes }),
      });
      const data = await res.json();
      if (data.success) {
        toast.success(`Created ${data.data.created} ticket${data.data.created !== 1 ? 's' : ''}`);
        setShowTicketModal(false); setSelectedIds([]); void fetchRecords();
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--ff-border-light)] bg-[var(--ff-bg-secondary)]">
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <input type="text" value={searchText} onChange={e => setSearchText(e.target.value)} placeholder="Search serial, DR, ticket..."
              className="pl-8 pr-3 py-1.5 rounded bg-[var(--ff-bg-primary)] border border-[var(--ff-border-light)] text-[var(--ff-text-primary)] text-sm w-56 placeholder:text-[var(--ff-text-tertiary)]" />
            {searchText && <Button variant="ghost" size="icon" onClick={() => setSearchText('')} className="absolute right-2 top-1/2 -translate-y-1/2" aria-label="Clear search"><XCircle className="w-3.5 h-3.5" /></Button>}
          </div>
      ...
      {selectedIds.length > 0 && (
        <div className="bg-blue-900/30 border-b border-blue-700 px-6 py-3 flex items-center justify-between">
          ...
          <div className="flex gap-3">
            ...
            <Button variant="primary" size="sm" onClick={() => setShowTicketModal(true)}><Wrench className="w-3.5 h-3.5" /> Create NOC Tickets</Button>
          </div>
        </div>
      )}

      <div className="flex-1 overflow-auto">
        {loading ? <LoadingSpinner className="h-64" size="lg" label="" /> : (
          <table className="w-full text-sm">
            ...
            <tbody>
              {records.map(record => <PPDataRow key={record.id} record={record} isSelected={selectedIds.includes(record.id)} onToggleSelect={toggleSelect} />)}
              {records.length === 0 && <tr><td colSpan={17} className="px-3 py-8 text-center text-[var(--ff-text-tertiary)]">No records found</td></tr>}
            </tbody>
          </table>
        )}
      </div>
