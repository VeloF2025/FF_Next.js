...
export function PhaseFinalDecision({
  ...
  // Auto-generate notes from selected reason codes (updates when codes change)
  const notesAutoGenerated = useRef(false);
  useEffect(() => {
    if (!decision || decision === 'PASS' || reasonCodes.length === 0) return;
    // Only auto-generate if the user hasn't manually typed notes
    if (notes && notesAutoGenerated.current === false) return;

    const action = decision === 'FAIL' ? 'REJECTED' : 'REWORK NEEDED';
    const issueLines = reasonCodes
      .map(code => `- ${REASON_CODE_LABELS[code]}`)
      .join('\n');
    const generated = `${action}: ${reasonCodes.length} issue(s) found.\n${issueLines}`;

    onNotesChange(generated);
    notesAutoGenerated.current = true;
  }, [reasonCodes, decision]); // eslint-disable-line react-hooks/exhaustive-deps

  // Reset auto-applied flag when decision changes back to null/PASS
  useEffect(() => {
    if (!decision || decision === 'PASS') {
      autoApplied.current = false;
      notesAutoGenerated.current = false;
    }
  }, [decision]);
