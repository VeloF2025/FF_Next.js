/**
 * SnagDetailPanel — Inline expanded snag detail (no modal overlay).
 * Contains the same inner content as SnagDetail but rendered inline
 * inside a table row expansion, not inside a fixed overlay.
 *
...
import { ExternalLink, Ticket, MapPin } from 'lucide-react';
...
/** Inline snag detail panel for table row expansion (no modal overlay) */
export function SnagDetailPanel({
  snag: initialSnag,
  photos,
  onUpdated,
  onPhotoAdded,
  onPhotoDeleted,
}: SnagDetailPanelProps) {
  ...
  const handlePoleLinked = useCallback(
    (updated: Snag) => {
      setSnag(updated);
      onUpdated(updated);
    },
  ...
  return (
    <div className="bg-zinc-900 border-t border-zinc-700 p-5 w-full">
      {/* Header */}
            ...
            return (
              <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                ...
                {distanceM != null && distanceM > 1 && (
                  ...
                  >
                    {distanceM < 1000
                      ? `${Math.round(distanceM)}m from pole`
                      : `${(distanceM / 1000).toFixed(1)}km from pole`}
                  </span>
                )}
                {distanceM != null && distanceM <= 1 && (
        ...
        return (
          <div className="flex items-center gap-2 flex-wrap mb-3 pb-3 border-b border-zinc-800">
            ...
            {actions?.reject && (
              <button
                type="button"
                onClick={() => { void handleStatusChange(actions.reject!.status); }}
                disabled={saving}
                className="text-xs bg-red-900 hover:bg-red-800 disabled:opacity-50 text-red-200 px-3 py-1.5 rounded transition-colors"
              >
      ...
      <div className="mt-3 border-t border-zinc-800 pt-3">
        <p className="text-xs text-zinc-500">
          Audit{' '}
          {snag.audit_date
            ? new Date(snag.audit_date).toLocaleDateString('en-ZA')
            : new Date(snag.created_at).toLocaleDateString('en-ZA')}
          {snag.report_number && <span className="ml-1">— {snag.report_number}</span>}
        </p>
        {snag.assigned_at && (
          <p className="text-xs text-zinc-500">
