 ...
 *
 * For one (drop_number, deduction_note, deduction-week) recovery episode, given
 * what we already recorded and whether FT is STILL deducting the DR in the bundle
 * currently being imported, decide the next action. No DB access — every branch is
 * unit-testable (mirrors classifyDeductionVerdict.ts / classifyReconRow.ts).
 *
 ...
 *   recovered / not_returned → noop              (terminal)
 */
...
export function decideRecovery(input: RecoveryDecisionInput): RecoveryAction {
  ...
  // Already resolved — recovery episodes are terminal once confirmed.
  if (existing.status !== 'pending') {
    return 'noop';
  }

  // A pending row only gets judged once a LATER bundle has arrived. ISO dates
  // compare lexically, so >= means "detected in this same import or later".
