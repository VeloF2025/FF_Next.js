...
const TEMPLATES: Template[] = [
  ...
  {
    name: 'overview — weekly totals CTE (summaries + exceptions + mismatches)',
    text: `
      WITH summaries AS (
        SELECT
      ...
      week_mismatches AS (
        ...
        WHERE ve.work_date >= $1::date
          AND ve.work_date <= $2::date
  ...
  {
    name: 'overview — top-OT this week (HAVING SUM > 0)',
    text: `
      SELECT ds.staff_id,
