...
class POService {
  ...
  async createPO(data: CreatePORequest): Promise<PurchaseOrder> {
    ...
    const response = await fetch(this.baseUrl, {
      ...
      body: JSON.stringify({
        ...
        items: sanitized.items?.map((item: CreatePORequest['items'][number] & { itemCode?: string; notes?: string }) => ({
  ...
  async createDeliveryNote(poId: string, deliveryData: {
    ...
    const deliveryNote: PODeliveryNote = {
      ...
      items: deliveryData.items.map((item) => ({
        ...
        quantityRejected: 0,
      })) as unknown as PODeliveryNote['items'],
  ...
  async createAmendment(poId: string, amendmentData: {
    ...
    const amendment: POAmendment = {
      ...
      changes: amendmentData.changes as unknown as POAmendment['changes'],
  ...
  private mapApiToFullPO(api: POApiDetail): PurchaseOrder & { items: POItem[] } {
    return {
      ...
      poNumber: api.poNumber,
      supplierId: api.supplierId as PurchaseOrder['supplierId'],
      title: `PO ${api.poNumber}`,
