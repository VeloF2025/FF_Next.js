...
const EXPECTED_TEST_URL = process.env.DATABASE_URL_TEST!;
...
describe('zone delivery migration 470', () => {
  ...
  it('enforces zone uniqueness, QA enums, row versions, and terminal handover', async () => {
    ...
    ).rejects.toThrow(/handed_over_at is immutable/);
  });
  ...
  it('adds the six action permissions only to super_admin and admin', async () => {
    ...
    expect(
      permissionRows.every(
        ({ parent_key, type }) => parent_key === 'construction-qa.qa-centre' && type === 'action'
      )
    ).toBe(true);
