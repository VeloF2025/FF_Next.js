...
describe('zone QA and automatic handover lifecycle', () => {
  ...
  async function registerZoneDocument(
  ...
  it('allows only one concurrent expected-version-zero zone projection create', async () => {
    const document = {
      ...key,
      documentType: 'fac' as const,
      documentSource: 'vf_storage' as const,
      filename: 'fac.pdf',
  ...
  it('hands over exactly once under concurrent recalculation with a complete evidence snapshot', async () => {
    ...
    expect(snapshot.documents).toEqual(expect.arrayContaining([
      expect.objectContaining({ documentType: 'test_pack', checksumSha256: '1'.repeat(64) }),
