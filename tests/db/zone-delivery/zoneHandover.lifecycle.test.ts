...
describe('zone QA and automatic handover lifecycle', () => {
  ...
  const setupLivePon = (initial?: ZoneDeliveryView) => harness.setupLivePon(initial);
  const registerZoneDocument = (
    view: ZoneDeliveryView,
    type: 'fac' | 'cac',
    checksum: string,
  ) => harness.registerZoneDocument(view, type, checksum);
  const recordQa = (
    view: ZoneDeliveryView,
    ...
    snagIds?: string[],
    reason?: string,
  ) => harness.recordQa(view, discipline, status, snagIds, reason);
  const waitForLock = (fragment: string) => harness.waitForLock(fragment);
  ...
  it('accepts only non-blocking maintenance linkage after handover without reversing dates', async () => {
    ...
    const { rows } = await pool.query<{
      ...
      SELECT handover_blocking, requires_reconfirmation, qa_discipline
      FROM zone_delivery_snag_links WHERE snag_id = $1
    `, [SNAG_ID]);
    ...
    const snapshot = await pool.query<{
      handover_snapshot: {
        snags: Array<{ snagId: string; qaDiscipline: string | null }>;
      };
    }>(`
      SELECT handover_snapshot FROM zone_delivery_state
      WHERE project_id = $1 AND zone_no = 1
    `, [PROJECT_ID]);
    expect(snapshot.rows[0]!.handover_snapshot.snags).toEqual([
      expect.objectContaining({ snagId: SNAG_ID, qaDiscipline: 'civil' }),
    ]);
    expect((await service.getActivity(key)).map(item => item.action))
      .toEqual(expect.arrayContaining(['civil_zone_qa_recorded', 'maintenance_linked']));
  });
