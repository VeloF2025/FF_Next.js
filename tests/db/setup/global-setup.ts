...
async function waitForReady(timeoutMs = 30_000): Promise<void> {
  ...
  export async function setup() {
    // Everything after the container starts is fallible — docker port, readiness
    // polling, nine file reads, nine queries. Vitest's globalSetup plugin calls
    // process.exit(1) straight from its error handler when setup() rejects, so
    // teardown() never runs and the container survives the process. On CI the
    // `if: always()` sweep catches it by label, but a local run has nothing to
    // reap it. Mirrors tests/db/velocity-review/global-setup.ts, which wraps its
