...
const SCHEMA = 'mig489_fleet_operational_status_rules_scratch';
...
const FORWARD = readFileSync(join(SQL_DIR, '489_fleet_operational_status_rules.sql'), 'utf8');
const ROLLBACK = readFileSync(join(SQL_DIR, 'rollback_489_fleet_operational_status_rules.sql'), 'utf8');
...
const PREREQUISITES = `
  ...
  INSERT INTO users (id, email) VALUES ('${USER}', 'migration-489@example.test');
...
describe('migration 489 operational status rules', () => {
  it('seeds the approved initial rule values', async () => {
    ...
    expect(rows).toEqual([{
      ...
      approaching_distance_meters: 10000, approaching_min_readings: 2, minimum_moving_speed_kmh: 5,
  ...
  it('enforces threshold checks and actor identity', async () => {
    ...
    await expect(db.query(`INSERT INTO fleet_operational_status_rules (version, timezone, effective_from,
      ...
      (3,'Africa/Johannesburg','2099-01-01',1,1,1,1,1,1,1,0,0,'00000000-0000-4000-8000-000000000000')`))
  ...
  it('rolls back only PR 4 rules and permission rows', async () => {
    await db.query(`INSERT INTO schema_migrations (filename) VALUES ('489_fleet_operational_status_rules.sql')`);
