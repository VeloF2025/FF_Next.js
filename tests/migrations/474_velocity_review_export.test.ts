...
const FORWARD = readFileSync(
  join(process.cwd(), 'scripts/migrations/sql/474_velocity_review_export.sql'),
  'utf8'
);
const ROLLBACK = readFileSync(
  join(process.cwd(), 'scripts/migrations/sql/rollback_474_velocity_review_export.sql'),
  'utf8'
);
const pool = new Pool({
...
dbDescribe('migration 474 applied to a scratch schema', () => {
  beforeAll(async () => {
  ...
  it('is idempotent and installs every ledger, check, and durable index', async () => {
    ...
    const tracker = await scoped<{ n: number }>(
      ...
      ['474_velocity_review_export.sql']
    );
      ...
      const rollbackState = await client.query<{
        ...
        `SELECT to_regclass('velocity_review_control')::text AS control_table,
                (SELECT COUNT(*)::int FROM schema_migrations
                 WHERE filename = '474_velocity_review_export.sql') AS tracker_count`
      );
